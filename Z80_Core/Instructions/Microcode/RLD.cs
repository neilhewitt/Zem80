...
namespace Z80.Core
{
    public class RLD : IInstructionImplementation
    {
        public ExecutionResult Execute(IProcessor cpu, InstructionPackage package)
        {
            ...
            a = a.SetBits(0, hl.GetBits(4, 4)); // high-order bits of (HL) to low-order bits of A
            hl = hl.SetBits(4, hl.GetBits(0, 4)); // low-order bits of (HL) to high-order bits of (HL)
            hl = hl.SetBits(0, aBits); // stored low-order bits of A to low-order bits of (HL)
