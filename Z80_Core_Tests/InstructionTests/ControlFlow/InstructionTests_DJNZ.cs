...
namespace Z80.Core.Tests
{
    ...
    {
        [Test]
        public void DJNZ([Values(0x00, 0x01, 0x50, 0x7F, -0x01, -0x80)] sbyte jump)
        {
            ushort address = 0x5000;
            Registers.PC = address;
            Registers.B = 1;
            if (jump == 0x7F) Registers.B = 0; // jump will not occur if value in B is zero (the NZ in DJNZ refers to the value of B, *not* the Zero flag)
