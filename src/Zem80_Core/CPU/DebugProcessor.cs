...
namespace Zem80.Core.CPU
{
    public class DebugProcessor
    {
        ...
        private Func<InstructionPackage, ExecutionResult> _executeInstruction;
        ...
        public ExecutionResult ExecuteDirect(byte[] opcode)
        {
            _cpu.Memory.Untimed.WriteBytesAt(_cpu.Registers.PC, opcode);
            DecodeResult result = _instructionDecoder.DecodeInstructionAt(_cpu.Registers.PC);
            _cpu.Registers.PC += result.InstructionPackage.Instruction.SizeInBytes;

            return _executeInstruction(result.InstructionPackage);
        }

        // execute an instruction directly (specified by mnemonic, so no decoding necessary)
        public ExecutionResult ExecuteDirect(string mnemonic, byte? arg1, byte? arg2)
        {
            ...
            return _executeInstruction(package);
        }

        public DebugProcessor(Processor cpu, Func<InstructionPackage, ExecutionResult> executeInstruction)
        {
            _cpu = cpu;
